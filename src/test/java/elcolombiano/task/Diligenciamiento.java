...
public class Diligenciamiento implements Task{
	...
	private List<List<String>> tablaInfo;
	
	
	public Diligenciamiento(List<List<String>> tablaInfo) {
	...
	public <T extends Actor> void performAs(T actor) {
		actor.attemptsTo(Enter.theValue( tablaInfo.get(0).get(0).toString().trim()  ).into(ElColombianoPage.EMAIL));
		actor.attemptsTo(Enter.theValue( tablaInfo.get(0).get(1).toString().trim()  ).into(ElColombianoPage.CONFIRMACION_CORREO));
		actor.attemptsTo(Enter.theValue( tablaInfo.get(0).get(2).toString().trim()  ).into(ElColombianoPage.PASSWORD));
		...
		if(tablaInfo.get(0).get(10).toString().trim().equals("Masculino"))
